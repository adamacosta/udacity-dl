"""
From the tensorflow Deep MNIST for Experts tutorial:
...
# convolution uses a stride of one and are zero padded so that the output 
# is the same size as the input. Our pooling is plain old max pooling over 
# 2x2 blocks. To keep our code cleaner, let's also abstract those operations 
# into functions.
def conv2d(x, W):
...
# the ReLU function, and finally max pool.
...
# and turn it off during testing. TensorFlow's tf.nn.dropout op automatically 
